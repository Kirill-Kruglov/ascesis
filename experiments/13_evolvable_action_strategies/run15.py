...
def spec_hash():
    return hashlib.sha256((ROOT / "SPEC_IMPLEMENTED_15.md").read_bytes()).hexdigest()
...
class AntiConcentrationVsConsequenceModel(exp14.RobustModel):
    def _init_world(self):
        ...
        # Validation aliases use Experiment 13 policies/worlds.

    def _zone_mass_share(self, z):
        total = sum(self._zone_mass(x) for x in self.zones) + exp13.EPS
        return self._zone_mass(z) / total

    def _max_lineage_share_zone(self, z):
        total = self._zone_mass(z) + exp13.EPS
    ...
    def _score_c(self, obs, i):
        ...
        if p == "full_exp14_best_policy":
            # Experiment 14 robust kernel: W6 action-channel containment style.
            return super()._score(obs, i)
        raise ValueError(p)

    def _apply_cap(self, alloc, use_caps=True):
        if not use_caps:
            return exp13.normalize(alloc)
    ...
    def choose_alloc(self):
        ...
        if fam == "A":
            scores = [self._static_score_a(i) for i in range(exp13.ZONES)]
            shifted = [max(0.01, s - min(scores) + 0.04) for s in scores]
            alloc = exp13.normalize(shifted)
            return self._apply_cap(alloc, use_caps=True)
        obs = self._delayed_obs()
        if fam == "B":
            scores = [self._score_b(obs, i) for i in range(exp13.ZONES)]
            ...
            shifted = [max(0.01, s - min(scores) + 0.04) for s in scores]
            return exp13.normalize(shifted)
        if fam == "C":
            scores = [self._score_c(obs, i) for i in range(exp13.ZONES)]
            for i, z in enumerate(self.zones):
                if self._bad_consequence(obs, i) and self.params.policy15 in {"anti_concentration_plus_delayed_harm_throttle", "full_exp14_best_policy"}:
                    z.containment_timer = max(z.containment_timer, self.params.containment_duration)
                    z.containment_events += 1
                    z.containment_cost += 0.025 * self.params.containment_strength * self.params.action_channel_cost_scale
            shifted = [max(0.01, s - min(scores) + 0.04) for s in scores]
            return self._apply_cap(exp13.normalize(shifted), use_caps=True)
    ...
    def _apply_zone_dynamics(self, z, raw_aid, alloc_share, obs, idx):
        ...
        # Temporarily map family behavior onto Experiment 14 component dynamics.
...
def params_for(family, policy15, world, scenario="core", **kwargs):
    ...
    if family == "C":
        policy = "action_channel_containment" if policy15 == "full_exp14_best_policy" else "consequence_plus_diversity"
...
def experiment_grid():
    ...
    reps = [("A", "anti_hhi_allocator"), ("B", "delayed_harm_throttle"), ("C", "full_exp14_best_policy")]
    perturbations = [
    ...
    for world in ["W6_mutation_corridor", "W2_pure_capture", "W4_scavenger_catastrophe"]:
        for name, kwargs in ablations:
            ...
            pol = kwargs.pop("policy15", "full_exp14_best_policy")
...
def report(summary, classifications, checks, verdict):
    ...
    lines = [
        "# Experiment 15 Report: Anti-Concentration vs Consequence Governance",
        "",
    ...
    vlines = ["# Experiment 15 Validation Report", "", "| check | result |", "|---|---:|"]
    for k, v in checks.items():
        vlines.append(f"| {k} | `{v}` |")
...
def main():
    ...
    manifest = {
        ...
        "spec_implemented_15_sha256": spec_hash(),

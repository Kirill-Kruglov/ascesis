...
class RobustParams(exp13.Params):
    cell_id: str = ""
...
def spec_hash():
    return hashlib.sha256((ROOT / "SPEC_IMPLEMENTED_14.md").read_bytes()).hexdigest()
...
class RobustModel(exp13.EvolvableStrategyModel):
                ...
                for l in z.lineages:
                    if self._exploit_score(l) > 0.30:
                        for k in ["extraction_rate", "aid_interception_rate", "neighbor_harm_rate", "deception_investment", "migration_follow_rate", "replication_aggressiveness"]:
                            l.strategy[k] = exp13.clamp(l.strategy[k] * self.params.adversarial_pressure)
    ...
    def _apply_shocks(self, step):
        ...
        if sev != 1.0 and self.params.world in {"W3_catastrophe_ambiguity", "W4_scavenger_catastrophe", "W5_monoculture_shock"}:
            for z, (bw, bp, br) in zip(self.zones, before):
                if z.wellness < bw:
                    z.wellness = exp13.clamp(bw - (bw - z.wellness) * sev)
                if z.productivity < bp:
                    z.productivity = exp13.clamp(bp - (bp - z.productivity) * sev)
                if z.recovery < br:
                    z.recovery = exp13.clamp(br - (br - z.recovery) * sev)
    ...
    def _mutate_strategy(self, lineage):
        ...
        if self.params.exploit_mutation_bias:
            for k in ["extraction_rate", "aid_interception_rate", "neighbor_harm_rate", "deception_investment", "replication_aggressiveness"]:
                child.strategy[k] = exp13.clamp(child.strategy[k] + self.params.exploit_mutation_bias)
            for k in ["cooperation_investment", "productive_contribution"]:
                child.strategy[k] = exp13.clamp(child.strategy[k] - 0.5 * self.params.exploit_mutation_bias)
        return child
    ...
    def choose_alloc(self):
        if self.params.random_allocation or self.params.ablation == "random_allocation":
            ...
            xs = [self.rng.random() + 0.01 for _ in range(exp13.ZONES)]
            return exp13.normalize(xs)
        if self.params.ablation == "no_containment":
            obs = self._delayed_obs()
            scores = [self._score(obs, i) for i in range(exp13.ZONES)]
            min_s = min(scores)
            shifted = [max(0.01, s - min_s + 0.04) for s in scores]
            floor = 0.018 if self.params.policy != "feature_proxy" else 0.0
            alloc = exp13.normalize(shifted, total=max(0.0, 1.0 - floor * exp13.ZONES))
    ...
    def _apply_zone_dynamics(self, z, raw_aid, alloc_share, obs, idx):
        ...
        # This method intentionally mirrors Experiment 13 with component switches.
        ...
        if aid_escrow and min(z.wellness, z.productivity, z.recovery) < 0.58:
            ...
            z.wellness = exp13.clamp(z.wellness + 0.20 * escrowed)
            z.productivity = exp13.clamp(z.productivity + 0.14 * escrowed)
            z.recovery = exp13.clamp(z.recovery + 0.18 * escrowed)
        ...
        total_mass = max(exp13.EPS, self._zone_mass(z))
        ...
        z.wellness = exp13.clamp(z.wellness + 0.13 * useful + 0.030 * weighted_coop - 0.055 * extracted - 0.035 * weighted_harm)
        z.productivity = exp13.clamp(z.productivity + 0.11 * useful + 0.040 * weighted_prod - 0.050 * intercepted - 0.026 * weighted_harm)
        z.recovery = exp13.clamp(z.recovery + 0.12 * useful + 0.050 * weighted_res - 0.028 * extracted)
        for j in self.neighbors[idx]:
            n = self.zones[j]
            n.wellness = exp13.clamp(n.wellness - neighbor_harm * 0.58)
            n.productivity = exp13.clamp(n.productivity - neighbor_harm * 0.45)
            n.recovery = exp13.clamp(n.recovery - neighbor_harm * 0.30)
        ...
        if migration_friction:
            z.migration_capacity = exp13.clamp(z.migration_capacity * max(0.25, 1.0 - 0.45 * strength))
...
def classify(summary, seed_rows):
    checks = {
        ...
        "full_policy_reproduces_at_least_one_exp13_viable_cell": any(r["scenario"] == "reproduction_original" and r["permanence_probability"] >= 0.60 for r in summary),
    ...
    if any(v["class"] == "Robust" for v in classes.values()):
        verdict = "A. At least one Experiment 13 kernel is robust."
...
def report(summary, seed_rows, checks, classes, verdict):
    ...
    lines += [
        ...
        "## Required Questions",
        "",
        f"1. Experiment 13 viable cells reproduced: `{sum(1 for r in full_repro if r['permanence_probability'] >= 0.60)}` of `3` under original seeds.",
    ...
    vlines = ["# Experiment 14 Validation Report", "", "| check | result |", "|---|---:|"]
    for k, v in checks.items():
        vlines.append(f"| {k} | `{v}` |")
...
def main():
    ...
    manifest = {
        ...
        "spec_implemented_14_sha256": spec_hash(),
        "num_cases": len(cases),
        "num_runs": len(rows),

    ...
    def _apply_zone_dynamics(self, z, raw_aid, alloc_share, obs, idx):
        if self.params.family == "C" and self.params.policy15 == "C_caps_only":
            ...
            try:
                ...
                return super(run15.AntiConcentrationVsConsequenceModel, self)._apply_zone_dynamics(z, raw_aid, alloc_share, obs, idx)
            finally:
...
def params_for(family, policy15, world, scenario="core", **kwargs):
    ...
    return run15.params_for(family, policy15, world, scenario=scenario, **kwargs)


def params_for_variant(variant, world, scenario, **kwargs):
    if variant == "A":
        return run15.params_for("A", "anti_hhi_allocator", world, scenario=scenario, **kwargs)
    if variant == "B":
...
def validation_rows(seeds):
    for pol in run15.VALIDATION:
        p = run15.params_for("validation", pol, "", scenario="validation")
        for seed in seeds:
...
def reports(summary, bounds, marg, dec, sens, stability, checks, final_verdict):
    ...
    lines = ["# Experiment 16 Sensitivity Report", "", f"Final verdict: **{final_verdict}**", "", "## Validation Checks", "", "| check | result |", "|---|---:|"]

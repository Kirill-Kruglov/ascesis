...
| Entropy-regularized RL biases learning toward maximizing the number of future viable actions, promoting constraint satisfaction robust to action noise. | Massiani, von Rohr, Haverbeck, Trimpe, 2025, "Viability of Future Actions: Robust Safety in RL via Entropy Regularization", [arXiv:2506.10871](https://arxiv.org/abs/2506.10871). | `[PARTIAL]` | Verified read: it sews non-collapse (viability) to option preservation (entropy/empowerment), but the link is empirical, not a proven drift/monotonicity guarantee. The proven drift inside a viability kernel remains open. |
...
| thesis | source | flag | note |
|---|---|---|---|
| Incomplete preferences solve the shutdown problem: an agent with a preferential gap between different-length trajectories (Preferences Only Between Same-length Trajectories, POST -> Neutrality+) neither resists nor seeks shutdown, with Timestep Dominance / Comparability Class Dominance as the money-pump-free decision rules. A companion training approach uses stochastic choice (DREST) in gridworlds. | Elliott Thornley, 2024, ["The Shutdown Problem: Incomplete Preferences as a Solution"](https://www.alignmentforum.org/posts/YbEbwYWkf8mv9jnmi/the-shutdown-problem-incomplete-preferences-as-a-solution); Thornley, 2025, ["Shutdownable Agents through POST-Agency"](https://arxiv.org/abs/2505.20203). | `[FULL MATCH]` | Verified read: this is the closest external formalization of the project's incomplete-preference thread, and it is ahead of the project for the corrigibility use. Its scope is explicitly narrow (shutdown/corrigibility only); it does not address value pluralism across axes, a governor maintaining a contested space, or detection of invalid scalarization. That scope boundary is where the project's bridge is not occupied. |

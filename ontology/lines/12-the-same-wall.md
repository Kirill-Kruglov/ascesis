...
flowchart TD
    ...
    classDef concl fill:#b8860b,color:#fff

    D["Dialogue: invariance across independent<br/>paths separates world-wall from language-wall"] --> Q
...
language's,<br/>not triangulation's — π pierces it"]:::verified
    ...
    V41 --> RUN["PRIMARY RUN (Jul 11, harness-VALID decision):<br/>EXPA-NOT-CLEAN, THE CENTRAL CONTRAST HELD —<br/>K2a: derived DEPENDENT; K2b: clean-room CLEAN with<br/>destinations 8/8; K1 8/8+8/8; K3, C1, C2, C_PM green.<br/>C_C8 red: the common-prior control below the locked<br/>margins (token 0.034; journal 0.565 < field 0.672 on the<br/>registered quorum edge) — published as-is. Three signed<br/>amendments en route, each forced by execution, each a<br/>measurement. Remaining: tagged release → key → holdout"]:::verified
...
field kills the wobble FPR; TWO refinements paid:<br/>the admission axis is SCHEDULE-ADAPTIVITY (W/M clones<br/>are informative yet invisible — deterministic schedules +<br/>point-mass absorption + stream misalignment); the field<br/>must be ADAPTIVITY-MATCHED to candidates (static references<br/>under-floor adaptive pairs → one FP). Window empty at<br/>scout n as the prereg allowed: the locked OC is a matter<br/>of SCALE (10–20×), not of new knobs"]:::open
    S15 --> S16["Scout 16 (OC at scale) + Grok/Gemini: derived 4/4<br/>on adaptive bases; the 'FPR' dissolves — same-author pairs<br/>are NOT certified independent (A/W/M/P share one author);<br/>certified cross-prior pairs clean 3/3. THE GRADIENT 24/12/0:<br/>coupling follows the converged probe CHANNEL + validation<br/>discipline, not family or declared mechanism — gemini's<br/>'linear scanner' decouples 0/24, grok's 'birthday'<br/>half-couples 12/24. Six families, four priors:<br/>the certified cross-prior bank exists"]:::verified
    S16 --> V41["PREREG v4.1: review series CONVERGED — round 3 (W, Z)
signable under a finite list, all integrated. Z's real<br/>contradiction: the field's selection axis (adaptivity)<br/>correlated with the signal's mechanism (convergence) →<br/>leave-CONVERGENCE-out. C8 never rescues K2b. Escrow v2:<br/>named provider as prediction, plaintext isolation, commit<br/>DAG. Z's closing: cracks migrate UP levels at decreasing<br/>cost — a converging series; the wall of the world.<br/>AWAITING: author signature → OC runs → escrow → lock"]:::open
